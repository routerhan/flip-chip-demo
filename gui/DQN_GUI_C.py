...
class WarpageEnv(gym.Env):
    def __init__(self, model, scaler_X, scaler_y, fixed_params, target_warpage=0.025):
        ...
        # 5) tool height：0.000 ~ 0.050，每 0.001
        self.tool_heights = [round(i * 0.001, 3) for i in range(51)]  # 51 種
    ...
    def step(self, action):
        ...
        tool_height = self.tool_heights[idx_tool]
...
def train_ddqn(substrate, copper_ratio, sbthk, material, target_warpage=0.025):
    ...
    magnet = int(inputs[1]) if len(inputs) > 1 else 0
    jig = float(inputs[2]) if len(inputs) > 2 else 0.0
    b1 = int(inputs[4]) if len(inputs) > 4 else 0
    w1 = int(inputs[5]) if len(inputs) > 5 else 0
    tool = float(inputs[0]) if len(inputs) > 0 else 0.0
    ...
    out.append(f" Tool高度: {tool:.3g} (mm)")
    out.append("")
